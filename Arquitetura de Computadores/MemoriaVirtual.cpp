...
int buscarMV(int,Page*,int*,int*,int,int*,int*);
...
int main()
{
    ...
    endereco = 42;
    resultadoBusca = buscarMV(endereco,pagina,memoriaRAM,memoriaDisco,tamanhoMemoriaRAM,&paginaUsada,&RAMUsada);
    ...
    cout << "Page Hit(s): " << pageHit << endl << "Page Miss: " << pageMiss << endl; 
}
...
int buscarMV(int end,Page *p, int *r, int *d, int tR, int *pU, int *rU)
{
        ...
        if((p+i)->enderecoVirtual == end)               //Verifica se o endereço está em uma delas, se estiver:
        {
            ...
            else if((p+i)->isDisco == true)             //Se está em Disco
            {
                ...
                else
                {
                    ...
                    //Portanto, vou só substituir no primeiro valor do vetor
